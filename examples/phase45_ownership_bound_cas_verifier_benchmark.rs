...
use un1c0::ownership_bound_cas_verifier::{
    OwnershipBoundCasVerifierMetrics, OwnershipBoundCasVerifierPipeline,
};
...
fn run_level(producers: u64) -> serde_json::Value {
    ...
    let worker_count = producers.min(8) as usize;
    let pipeline = Arc::new(
        OwnershipBoundCasVerifierPipeline::new(coordinator, worker_count, jobs as usize + 1)
            .expect("pipeline"),
    );
        ...
        handles.push(thread::spawn(move || {
            ...
            for offset in 0..JOBS_PER_PRODUCER {
                ...
                let ticket = pipeline
                    ...
                    .expect("verification queue admission");
    ...
    let metrics: OwnershipBoundCasVerifierMetrics = pipeline.metrics();
    assert_eq!(outcomes.len(), jobs as usize);
    assert_eq!(metrics.submitted_intents, jobs);
    assert_eq!(metrics.pre_admitted_intents, jobs);
    assert_eq!(metrics.pre_admission_failures, 0);
    assert_eq!(metrics.completed_intents, jobs);
    assert_eq!(metrics.failed_intents, jobs - 1);
    assert_eq!(metrics.verification_queue_full_rejections, 0);
    assert_eq!(metrics.latency_sample_count, jobs as usize);
    let throughput = if wall_us > 0.0 {
        jobs as f64 / (wall_us / 1_000_000.0)
    } else {
        0.0
    };
    let mut pipeline = Arc::try_unwrap(pipeline).expect("pipeline ownership");
    ...
    json!({
        ...
        "failed_conflicts": jobs - 1,
        "verification_queue_full_rejections": metrics.verification_queue_full_rejections,
        "wall_us": wall_us,
        "throughput_intents_per_sec": throughput,
        "verification_wait_p50_us": metrics.verification_wait_p50_us,
        "verification_wait_p95_us": metrics.verification_wait_p95_us,
        "verification_wait_max_us": metrics.verification_wait_max_us,
        ...
        "mutation_service_p50_us": metrics.mutation_service_p50_us,
        "mutation_service_p95_us": metrics.mutation_service_p95_us,
        "mutation_service_max_us": metrics.mutation_service_max_us,
...
fn main() {
    ...
    println!(
        ...
        json!({
            "phase": 45,
            "workload": "bounded parallel pre-admission verification with same-generation CAS contention",
            "jobs_per_producer": JOBS_PER_PRODUCER,
            "mutation_workers": 1,
            "secret_material_recorded": false,
            "cluster_mutation_performed": false,
            "production_boundary": "local bounded verifier pool, ordered dispatcher, and filesystem-backed mutation worker only",
            "results": results,
        })
    );
}
