...
use un1c0::emission_diagnostic_stream::{EmissionDiagnosticStream, MAX_STREAM_BYTES};
...
const FRAME_COUNTS: &[usize] = &[1, 2, 4, 8, 16, 32];
const UNITS: usize = 4;
...
struct BenchmarkRow {
    ...
    construct_p50_ns: u128,
    construct_p95_ns: u128,
    construct_p99_ns: u128,
    verify_p50_ns: u128,
    verify_p95_ns: u128,
    verify_p99_ns: u128,
    errors: usize,
...
fn main() {
    ...
    for &frames in FRAME_COUNTS {
        let reports = vec![report.clone(); frames];
        let stream = EmissionDiagnosticStream::from_verified_reports(
            70,
            &reports,
            &snapshot,
            &profile,
        ...
        .expect("bounded stream construction");
        let encoded = stream.to_json().expect("stream encoding");
        assert!(encoded.len() <= MAX_STREAM_BYTES);
        let construction = measure(SAMPLES, || {
            black_box(
        ...
        let verification = measure(SAMPLES, || {
            stream
                ...
                .expect("current-state stream verification");
        });
        rows.push(BenchmarkRow {
            ...
            total_stream_bytes: encoded.len(),
            bytes_per_frame: stream.total_frame_bytes() / frames,
            construct_p50_ns: percentile(&construction, 0.50),
            construct_p95_ns: percentile(&construction, 0.95),
            construct_p99_ns: percentile(&construction, 0.99),
