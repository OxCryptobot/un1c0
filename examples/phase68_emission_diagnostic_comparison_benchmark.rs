...
use un1c0::emission_diagnostic_comparison::EmissionDiagnosticComparison;
...
struct BenchmarkRow {
    ...
    after_observations: usize,
    functions_per_unit: usize,
    total_functions: usize,
    samples: usize,
    target: &'static str,
    comparison_p50_ns: u128,
    comparison_p95_ns: u128,
...
fn main() {
    ...
    let before_receipts = vec![receipt.clone()];
    let before =
        EmissionDiagnosticReport::from_receipts(&before_receipts, &snapshot, &profile, &candidates)
            .expect("before report");

    let mut rows = Vec::new();
    for &after_observations in OBSERVATIONS {
        let after_receipts = vec![receipt.clone(); after_observations];
        ...
        .expect("after report");
        let samples = measure(SAMPLES, || {
            EmissionDiagnosticComparison::compare(
                &before,
                &after,
                &snapshot,
                &profile,
        ...
        let comparison = EmissionDiagnosticComparison::compare(
            &before,
            &after,
            &snapshot,
            &profile,
        ...
        rows.push(BenchmarkRow {
            ...
            comparison_p50_ns: percentile(&samples, 0.50),
            comparison_p95_ns: percentile(&samples, 0.95),
            observation_delta: delta.observation_delta(),
            chunk_delta: delta.chunk_delta(),
            byte_delta: delta.byte_delta(),
            digest_equal: delta.digest_equal(),
            errors: 0,
