...
struct BenchmarkRow {
    ...
    target: &'static str,
    envelope_id: u64,
    raw_batch_p50_ns: u128,
    raw_batch_p95_ns: u128,
    envelope_refresh_p50_ns: u128,
    envelope_refresh_p95_ns: u128,
    errors: usize,
...
fn main() {
    ...
    for &units in UNIT_LEVELS {
        let raw_samples = measure(SAMPLES, || {
            let (mut session, batch) = create_batch(units, profile.clone());
            session
                .refresh_batch(&batch, &profile)
                .expect("valid raw batch refresh");
        });
        let envelope_samples = measure(SAMPLES, || {
            let (mut session, envelope) = create_envelope(units, profile.clone());
            session
                .refresh_envelope(&envelope, &profile)
                .expect("valid envelope refresh");
        });
        rows.push(BenchmarkRow {
            ...
            envelope_id: 1,
            raw_batch_p50_ns: percentile(&raw_samples, 0.50),
            raw_batch_p95_ns: percentile(&raw_samples, 0.95),
            envelope_refresh_p50_ns: percentile(&envelope_samples, 0.50),
            envelope_refresh_p95_ns: percentile(&envelope_samples, 0.95),
            errors: 0,
...
fn create_batch(
    ...
    let starts = fixture
        ...
        .collect();
    let session = SemanticBatchSession::start(profile, starts).unwrap();
    let updates = fixture
        .iter()
        .map(|unit| SemanticEditUpdate {
            unit: unit.id.clone(),
            ueg: unit.changed.clone(),
            manifest: SemanticBatchSession::manifest_for(
                &session,
                &unit.id,
                vec![unit.leaf_range.clone()],
            )
            .unwrap(),
        })
        .collect();
...
fn create_envelope(
    units: usize,
    profile: TargetCapabilityProfile,
) -> (SemanticBatchSession, SemanticBatchEnvelope) {
    let (session, batch) = create_batch(units, profile);
