...
fn main() {
    ...
    for &observations in OBSERVATIONS {
        ...
        let bundle_samples = measure(SAMPLES, || {
            EmissionEvidenceBundle::from_receipts(&receipts).expect("bundle construction");
        });
        let bundle = EmissionEvidenceBundle::from_receipts(&receipts).expect("bundle construction");
        let verification_samples = measure(SAMPLES, || {
            bundle
                .verify_for(&snapshot, &profile, &candidates)
                .expect("bundle verification");
        });
        rows.push(BenchmarkRow {
            ...
            bundle_p50_ns: percentile(&bundle_samples, 0.50),
            bundle_p95_ns: percentile(&bundle_samples, 0.95),
