...
use un1c0::emission_receipt_aggregate::EmissionReceiptAggregate;
use un1c0::semantic::TargetCapabilityProfile;
use un1c0::semantic_batch::{
    SemanticBatchEnvelope, SemanticEditBatch, SemanticEditUpdate, SemanticUnitId, SemanticUnitStart,
};
...
const OBSERVATION_LEVELS: &[usize] = &[1, 2, 4, 8];
...
struct BenchmarkRow {
    ...
    samples: usize,
    target: &'static str,
    aggregate_p50_ns: u128,
    aggregate_p95_ns: u128,
...
fn main() {
    ...
    let (session, candidates, batch_envelope) = prepared_state(profile.clone());
    let snapshot = SemanticSnapshotEnvelope::capture(&session, batch_envelope.batch_id()).unwrap();
    ...
    for &observations in OBSERVATION_LEVELS {
        let receipts = vec![receipt.clone(); observations];
        let aggregate_samples = measure(SAMPLES, || {
            EmissionReceiptAggregate::from_receipts(&receipts).expect("aggregate receipts");
        });
        ...
        rows.push(BenchmarkRow {
            ...
            aggregate_p50_ns: percentile(&aggregate_samples, 0.50),
            aggregate_p95_ns: percentile(&aggregate_samples, 0.95),
            verification_p50_ns: percentile(&verification_samples, 0.50),
            verification_p95_ns: percentile(&verification_samples, 0.95),
            chunks_emitted: aggregate
                .as_ref()
                .map(|value| value.chunks_emitted())
                .unwrap_or(stats.chunks_emitted),
            errors: 0,
...
fn prepared_state(
    ...
    let starts = fixture
        ...
        .collect();
    let mut session = un1c0::SemanticBatchSession::start(profile.clone(), starts).unwrap();
    let updates = fixture
        .iter()
